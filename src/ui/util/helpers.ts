...
const formatPlayerAge = (
	age: number,
	collegeClass?: "Fr" | "So" | "Jr" | "Sr",
): string => {
	if (collegeClass) {
		return `${age} (${collegeClass})`;
	}
	return String(age);
};

const getCollegeClassForSeason = (
	bornYear: number,
	draftYear: number,
	season: number,
): "Fr" | "So" | "Jr" | "Sr" | undefined => {
	// Calculate age during the season
	const age = season - bornYear;

	// College basketball players are typically 18-22
	if (age < 18 || age > 22) {
		return undefined;
	}

	// Map age to class year
	if (age === 18) return "Fr";
	if (age === 19) return "So";
	if (age === 20) return "Jr";
	if (age >= 21) return "Sr";
