...
import { RedshirtBadge } from "../../components/PlayerNameLabels.tsx";
import {
	confirm,
	helpers,
	toWorker,
...
> & {
	...
	if (season !== undefined || showContract) {
		// Check if this is a scholarship system
		const isScholarshipSystem = player.contract.status !== undefined || player.contract.amount === 0;

		if (isScholarshipSystem && (season === undefined || season === currentSeason)) {
			// Scholarship display - just show years remaining
			const yearsRemaining = player.contract.yearsRemaining;

			contractInfo = yearsRemaining !== undefined ? (
				<>
					{yearsRemaining} {yearsRemaining === 1 ? "yr" : "yrs"}
					<br />
				</>
			) : null;
		} else {
			// Legacy contract display
			let amount;
			let exp;
	...
	if (retired && season === undefined) {
		...
		statusInfo = (
			<div className="d-flex align-items-center">
				...
				{isRedshirt ? <RedshirtBadge /> : null}
				<SkillsBlock
					className={injured || isRedshirt ? undefined : "skills-alone"}
					skills={skills}
				/>
				<WatchBlock
					className="ms-2"
	...
	// For college basketball, show high school instead of college
	const highSchool =
				...
				<div className="player-bio">
					<div className="d-flex">
						...
						<div>
							...
							<br />
							{isSport("hockey") && highSchool === "None" ? null : (
								<>
									High School:{" "}
									<a
										href={helpers.leagueUrl([
											...
											"most",
											"college",
											window.encodeURIComponent(highSchool),
										])}
									>
										{highSchool}
									</a>
							...
							Class:{" "}
							{player.tid === PLAYER.UNDRAFTED ? (
								// Draft prospects - show high school class based on when they're draftable
								player.draft.year === currentSeason ? "HS Senior"
								: player.draft.year === currentSeason + 1 ? "HS Junior"
								: player.draft.year === currentSeason + 2 ? "HS Sophomore"
								: "HS Freshman"
							) : (() => {
								// College players - show college class based on age
								const className = player.age === 18
									? "Freshman"
									: player.age === 19
										? "Sophomore"
										: player.age === 20
								...
								// Add (RS) suffix if player has redshirted in the past (but not currently redshirting)
								if (className !== "Unknown" && player.contract?.hasRedshirted && !player.contract?.isRedshirt) {
									return `${className} (RS)`;
								}
								return className;
							})()}
							...
							{player.ageAtDeath === null ? (
								<>
									Age: {player.age}
									{(player as any).collegeClass ? ` (${(player as any).collegeClass})` : ""}
									<br />
								</>
							) : (
								<>
