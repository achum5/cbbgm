...
export type GameAttributesLeague = {
	...
	maxScholarships: number; // Maximum scholarship players (college basketball only)
	minContract: number;
...
export type MoodComponents = {
	prestige: number; // School prestige rating (0-100, blue bloods 95-98)
	facilities: number; // Facilities level (coaching, health, etc.)
	teamPerformance: number; // Regular season win/loss record
	tournament: number; // Tournament success (conference + NCAA tournament)
	homeProximity: number; // Distance from hometown (in-state bonus)
	playingTime: number; // Actual minutes played vs expected
	difficulty: number; // Game difficulty setting
	relatives: number; // Family connections on team
	custom?: {
		text: string;
		amount: number;
	}[];
...
export type MoodTrait = "P" | "T" | "W" | "H"; // P=Prestige, T=Playing Time, W=Winning, H=Hometown
...
export type PlayerScholarship = {
	// New scholarship fields (optional for backward compatibility)
	status?: "scholarship" | "walk-on"; // Default to "scholarship" if not specified
	yearsRemaining?: number; // 1-4 years of eligibility remaining
	originalClass?: number; // Recruiting class year (season they joined the team)
	isRedshirt?: boolean; // Currently redshirting this season (doesn't use eligibility)
	hasRedshirted?: boolean; // Has ever used their one-time redshirt year

	// Backward compatibility fields (required for now to avoid breaking existing code)
	// These will be made optional/removed in a future cleanup
	amount: number; // Always 0 in college basketball
	exp: number; // Expiration year
	rookie?: true; // Maps to originalClass === current season
	rookieResign?: true; // Not used in college
};

// Keep PlayerContract as alias for backward compatibility during transition
export type PlayerContract = PlayerScholarship;
...
export type PlayerWithoutKey<PlayerRatings = any> = {
	...
	college: string;
	highSchool?: string; // High school the player attended before college
	collegeClass?: "Fr" | "So" | "Jr" | "Sr"; // Freshman, Sophomore, Junior, Senior
	scholarship: PlayerScholarship & {
		temp?: true; // Used only on import
	};
	// Backward compatibility alias (required for now, will be made optional later)
	contract: PlayerScholarship & {
		temp?: true;
	};
	customMoodItems?: {
		amount: number;
...
export type ConferenceTournament = {
	cid: number;
	currentRound: number;
	complete: boolean;
	champion?: number; // tid of tournament winner
	series: {
		home: PlayoffSeriesTeam;
		away?: PlayoffSeriesTeam;
		gids?: number[];
...
export type ConferenceTournaments = {
	season: number;
	tournaments: ConferenceTournament[];
};
...
export type Team = {
	...
	pop: number;
	state?: string; // US state abbreviation for college teams
	prestigeBase?: number; // 0-100 historical baseline prestige (never changes)
	prestige?: number; // 0-100 current dynamic prestige (changes based on performance)
...
export type TeamSeasonWithoutKey = {
	...
	hype: number;
	prestige?: number; // 0-100 current prestige at end of this season
	pop: number;
	stadiumCapacity: number;
	revenues: {
