...
const updatePlayers = async (
	...
	if (
		...
		if (type === "at_pick") {
			...
			if (arg === undefined) {
				throw new Error("High school must be specified in the URL");
			}

			title = `Best Players From ${arg}`;
			description = `These are the best players from high school ${arg}. <a href="${helpers.leagueUrl(
				["frivolities", "colleges"],
			)}">Other high schools.</a>`;

			filter = (p) => {
				const highSchool =
					p.highSchool && p.highSchool !== "" ? p.highSchool : "None";
				return highSchool === arg;
			};
			getValue = playerValue;
