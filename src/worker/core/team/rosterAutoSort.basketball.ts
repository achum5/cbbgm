...
const rosterAutoSort = async (tid: number, onlyNewPlayers?: boolean) => {
	...
	// Separate redshirt players from active players
	const activePlayers = players.filter(p => {
		const player = playersFromCache.find(pc => pc.pid === p.pid);
		return !player?.scholarship?.isRedshirt;
	});
	const redshirtPlayers = players.filter(p => {
		const player = playersFromCache.find(pc => pc.pid === p.pid);
		return player?.scholarship?.isRedshirt;
	});

	// Sort active players normally
	const rosterOrders = getRosterOrderByPid(activePlayers, tid, true);

	// Add redshirt players at the end
	let redshirtStartOrder = activePlayers.length;
	for (const p of redshirtPlayers) {
		rosterOrders.set(p.pid, redshirtStartOrder);
		redshirtStartOrder++;
	}
