...
const ovr = (
	...
	// Check if we're in college basketball mode (salaryCapType === "none")
	const isCollegeMode = g.get("salaryCapType") === "none";

	if (playoffs) {
		a = 0.6388;
		b = -0.2245;
		// College basketball players have lower OVRs (30-55 range) than professional (40-90 range)
		// Use adjusted k constant to normalize team ratings to 0-100 scale
		k = isCollegeMode ? 106 : 157.43;
	} else {
		a = 0.3334;
		b = -0.1609;
		// College basketball uses k = 69 to center average team at ~50 OVR
		// Professional basketball uses k = 102.98
		k = isCollegeMode ? 69 : 102.98;
	}
