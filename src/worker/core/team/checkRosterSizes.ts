...
const checkRosterSizes = async (
	...
	const checkRosterSize = async (tid: number, userTeamAndActive: boolean) => {
		...
		// Count scholarship players
		const numScholarshipPlayers = players.filter(
			p => p.scholarship?.status === "scholarship" || p.contract?.status === "scholarship"
		).length;

		// Check scholarship limit first
		if (numScholarshipPlayers > g.get("maxScholarships")) {
			if (userTeamAndActive) {
				...
				userTeamSizeError += `more than the maximum number of scholarship players (${g.get(
					"maxScholarships",
				)}). You currently have ${numScholarshipPlayers} scholarship players. You must convert players to walk-ons or remove them (by <a href="${helpers.leagueUrl([
					"roster",
				])}">releasing them from your roster</a> or through <a href="${helpers.leagueUrl(
					["trade"],
				)}">trades</a>) before continuing.`;
			} else {
				// AI teams: convert excess scholarship players to walk-ons (lowest value first)
				const scholarshipPlayers = players
					.filter(p => p.scholarship?.status === "scholarship" || p.contract?.status === "scholarship")
					.sort((a, b) => a.value - b.value); // Lowest value first

				const numToConvert = numScholarshipPlayers - g.get("maxScholarships");
				for (let i = 0; i < numToConvert && i < scholarshipPlayers.length; i++) {
					const p = scholarshipPlayers[i];
					if (!p) continue; // Safety check

					if (p.scholarship) {
						p.scholarship.status = "walk-on";
					}
					if (p.contract) {
						p.contract.status = "walk-on";
					}
		...
		if (numPlayersOnRoster > g.get("maxRosterSize")) {
			...
			if (userTeamAndActive) {
				...
				userTeamSizeError += `less than the minimum number of players (${g.get(
					...
					"free_agents",
				])}">recruiting</a> or <a href="${helpers.leagueUrl([
					"trade",
				])}">trades</a>) before continuing.<br><br>Reminder: you can always sign walk-ons to fill your roster!`;
			} else {
				// Auto-add players
	...
	// List of free agents (walk-ons), sorted by value. This is used to bump teams up to the minimum roster size.
	for (const p of players) {
		// In college basketball, free agents are walk-ons
		if ((p.scholarship?.status === "walk-on" || p.contract?.status === "walk-on") ||
		    (!p.scholarship && !p.contract)) {
