...
const newSchedule = (
	...
	if (Object.hasOwn(g, "groupScheduleSeries") && g.get("groupScheduleSeries")) {
		...
		// Helper function to group games into days (so teams don't play multiple times per day)
		const groupIntoDays = (gamesInput: [number, number][]) => {
			...
			for (let i = 0; i < games.length; i++) {
				...
				for (let j = 0; j <= jMax; j++) {
					if (
						!tidsInDays[j]!.includes(games[i]![0]) &&
						!tidsInDays[j]!.includes(games[i]![1])
					) {
						tidsInDays[j]!.push(games[i]![0], games[i]![1]);
						days[j]!.push(games[i]!);
						used = true;
						break;
					}
				...
				if (!used) {
					days.push([games[i]!]);
					tidsInDays.push([games[i]![0], games[i]![1]]);
					jMax += 1;
				}
			}
		...
		// For college basketball: split into non-conference (first half) and conference (second half) games
		if (isSport("basketball")) {
			const teamsByCid = new Map<number, number>();
			for (const t of teams) {
				teamsByCid.set(t.tid, t.seasonAttrs.cid);
			}

			const nonConferenceGames: [number, number][] = [];
			const conferenceGames: [number, number][] = [];

			for (const game of tids) {
				const homeCid = teamsByCid.get(game[0]);
				const awayCid = teamsByCid.get(game[1]);

				if (homeCid === awayCid) {
					conferenceGames.push(game);
				} else {
					nonConferenceGames.push(game);
				}
			}

			// Group each section into days, then concatenate: non-conference first, then conference
			const nonConfDays = groupIntoDays(nonConferenceGames);
			const confDays = groupIntoDays(conferenceGames);

			tids = [...nonConfDays, ...confDays];
		} else {
			// Order the schedule so that it takes fewer days to play
			tids = groupIntoDays(tids);
		}
	}
