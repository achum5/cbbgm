...
const normalizeContractDemands = async ({
	...
	for (const info of playerInfosToUpdate) {
		...
		// For college basketball (salaryCapType: "none"), preserve scholarship format
		if (isSport("basketball") && g.get("salaryCapType") === "none") {
			// Calculate years remaining based on age (most reliable method)
			// Age 18 or younger = Freshman = 4 years
			// Age 19 = Sophomore = 3 years
			// Age 20 = Junior = 2 years
			// Age 21+ = Senior = 1 year
			const age = g.get("season") - p.born.year;
			...
			p.contract = {
				status: p.scholarship?.status || (p.tid >= 0 ? "scholarship" : "walk-on"),
				yearsRemaining,
				originalClass: p.scholarship?.originalClass || p.draft?.year || g.get("season"),
				isRedshirt: p.scholarship?.isRedshirt ?? false,
				hasRedshirted: p.scholarship?.hasRedshirted ?? false,
				amount: 0,
				exp: g.get("season") + yearsRemaining,
			};
			// Keep scholarship and contract in sync
		...
		if (p.tid === PLAYER.FREE_AGENT && p.contract.exp < minNewContractExp) {
			...
			// Update yearsRemaining for scholarship system - don't recalculate, already set correctly above
			if (isSport("basketball") && g.get("salaryCapType") === "none" && p.contract.yearsRemaining !== undefined) {
				// Just update exp to match years remaining (don't recalculate years)
				p.contract.exp = g.get("season") + p.contract.yearsRemaining;
				p.scholarship = p.contract;
			}
		}
