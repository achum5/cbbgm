...
const newPhasePreseason = async (
	...
	for (const p of players) {
		// MIGRATION: Convert old contract system to scholarship system (college basketball)
		if (
			isSport("basketball") &&
			p.tid >= 0 &&
			p.contract &&
			!p.scholarship?.status
		) {
			// Player has old contract format, needs migration to scholarship
			const yearsRemaining = p.collegeClass
				? p.collegeClass === "Fr"
					? 4
					: p.collegeClass === "So"
						? 3
						: p.collegeClass === "Jr"
							? 2
							: 1
				: 4; // Default to 4 if no class

			// Convert to scholarship format
			p.scholarship = {
				status: "scholarship",
				yearsRemaining,
				originalClass: newSeason - (4 - yearsRemaining),
				amount: 0,
				exp: newSeason + yearsRemaining,
			};

			// Also update the contract alias to point to the same object
		...
		if (repeatSeason) {
			...
			// Advance college class
			if (isSport("basketball") && p.collegeClass) {
				...
				// Seniors stay as Sr (will retire after this season based on forceRetireSeasons)
			}

			// Decrease scholarship eligibility for players on teams (college basketball)
			if (isSport("basketball") && p.tid >= 0 && p.scholarship) {
				// Check if player was redshirted last season BEFORE we reset the flag
				const wasRedshirtedLastSeason = p.scholarship.isRedshirt ?? false;

				// Reset redshirt status for new season (players who were redshirted last year are now active)
				// But keep hasRedshirted flag so we can show (RS) suffix
				if (p.scholarship.isRedshirt) {
					p.scholarship.isRedshirt = false;
				}
				// Also reset contract field for backward compatibility
				if (p.contract && p.contract.isRedshirt) {
					p.contract.isRedshirt = false;
				}

				// Decrease years remaining ONLY if player was NOT redshirted last season
				// Redshirted players preserve a year of eligibility
				if (p.scholarship.yearsRemaining !== undefined) {
					// Only decrease if they weren't redshirted last season
					if (!wasRedshirtedLastSeason) {
						p.scholarship.yearsRemaining -= 1;
					}

					// Graduate players with no eligibility remaining
					if (p.scholarship.yearsRemaining <= 0) {
						// Save team before changing status
						const wasOnUserTeam = p.tid === g.get("userTid");
						const previousTid = p.tid;

						// Player has exhausted eligibility - becomes draft eligible
						p.tid = PLAYER.UNDRAFTED;
						p.scholarship.yearsRemaining = 0;
						...
						logEvent(
							{
								...
								text: `<a href="${helpers.leagueUrl(["player", p.pid])}">${
									...
									p.lastName
								}</a> completed their college eligibility and will enter the draft.`,
								showNotification: wasOnUserTeam,
								pids: [p.pid],
								tids: [previousTid],
								score: 20,
							},
				...
				// Sync yearsRemaining with college class for consistency
				if (p.collegeClass && p.scholarship.yearsRemaining !== undefined) {
					if (p.collegeClass === "Fr" && p.scholarship.yearsRemaining !== 4) {
						p.scholarship.yearsRemaining = 4;
					} else if (p.collegeClass === "So" && p.scholarship.yearsRemaining !== 3) {
						p.scholarship.yearsRemaining = 3;
					} else if (p.collegeClass === "Jr" && p.scholarship.yearsRemaining !== 2) {
						p.scholarship.yearsRemaining = 2;
					} else if (p.collegeClass === "Sr" && p.scholarship.yearsRemaining !== 1) {
						p.scholarship.yearsRemaining = 1;
					}
				}

				// Sync contract fields with scholarship fields for backward compatibility
				if (p.contract) {
					p.contract.yearsRemaining = p.scholarship.yearsRemaining;
					p.contract.hasRedshirted = p.scholarship.hasRedshirted;
				}
			}
		}
