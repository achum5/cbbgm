...
const play = async (
	...
	const cbNoGames = async (playoffsOver: boolean = false, conferenceTournamentsOver: boolean = false) => {
		await updateStatus("Saving...");
		await idb.cache.flush();
		await updateStatus("Idle");
		await lock.set("gameSim", false);

		// Check to see if the season phases should advance
		const schedule = await season.getSchedule();

		// Regular season is over - transition to playoffs
		// Only transition when schedule is completely empty (all regular season games played)
		if (g.get("phase") === PHASE.REGULAR_SEASON && schedule.length === 0) {
			await phase.newPhase(
				PHASE.AFTER_TRADE_DEADLINE,
				conditions,
				gidOneGame !== undefined,
			);
		} else if (conferenceTournamentsOver) {
			await phase.newPhase(
				PHASE.PLAYOFFS,
				conditions,
				gidOneGame !== undefined,
	...
	const cbSaveResults = async (results: GameResults[], dayOver: boolean) => {
		...
		const conferenceTournamentsOver =
		...
		if (numDays - 1 <= 0 || playoffsOver || conferenceTournamentsOver) {
			await cbNoGames(playoffsOver, conferenceTournamentsOver);
		} else {
