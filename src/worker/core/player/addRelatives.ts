...
const makeSimilar = async (existingRelative: Player, newRelative: Player) => {
	...
	if (existingRelative.highSchool && Math.random() < 0.5) {
		newRelative.highSchool = existingRelative.highSchool;
	}
...
const applyNewCountry = async (p: Player, relative: Player) => {
	...
	if (newCountry) {
		const { college, highSchool, firstName, race } = await player.name(
			relativeCountry,
		);
