...
// Repeatedly simulate aging up to 29 (or 22 for college basketball), and pick the 75th percentile max
export const monteCarloPot = async ({
	...
	// For college basketball, simulate development only until age 22 (graduation age)
	// For professional basketball, simulate until age 29
	const maxAge = isSport("basketball") && g.get("salaryCapType") === "none" ? 22 : 29;
	...
	for (let i = 0; i < NUM_SIMULATIONS; i++) {
		...
		// Simulate aging until maxAge (22 for college, 29 for pro)
		for (let ageTemp = age + 1; ageTemp <= maxAge; ageTemp++) {
