...
const updatePlayMenu = async () => {
	...
	const allOptions = {
		...
		untilConferenceTournaments: {
			label: "Until conference tournaments",
			key: "r",
		},
		viewAllStar: {
	...
	if (
		...
		// For college basketball, just show "Until playoffs" during regular season
		// which will take them to conference tournaments
		if (allStarIndex > 0) {
			untilMore.push("untilAllStarGame");
		}

		// Add "Until playoffs" option which goes to conference tournaments
		untilMore.push("untilPlayoffs");
