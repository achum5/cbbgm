...
const addTeam = async () => {
	...
	return {
		...
		prestigeBase: t.prestigeBase ?? 70,
		prestige: t.prestige ?? t.prestigeBase ?? 70,
...
const updatePlayerRedshirt = async ({
	pid,
	isRedshirt,
}: {
	pid: number;
	...
	if (p && p.contract) {
		p.contract.isRedshirt = isRedshirt;
		if (p.scholarship) {
			p.scholarship.isRedshirt = isRedshirt;
			// When setting to redshirt, also mark that they have used their redshirt
			if (isRedshirt) {
				p.scholarship.hasRedshirted = true;
			}
		}
		// Also update contract for backward compatibility
		if (isRedshirt && p.contract) {
			p.contract.hasRedshirted = true;
		}
		await idb.cache.players.put(p);
		await toUI("realtimeUpdate", [["playerMovement"]]);
	}
...
const updateTeamInfo = async ({
	...
	for (const t of teams) {
		...
		t.prestigeBase = helpers.localeParseFloat(newTeam.prestigeBase as string);
		t.prestige = helpers.localeParseFloat(newTeam.prestige as string);
		...
		if (Number.isNaN(t.prestigeBase)) {
			throw new Error("Invalid prestigeBase");
		}

		if (Number.isNaN(t.prestige)) {
			throw new Error("Invalid prestige");
		}
